...
const App: React.FC = () => {
    ...
    const showToast = useCallback((msg: string, type: 'alert' | 'info' | 'error' | 'success' | 'warning' = 'info') => {
    ...
    useEffect(() => {
        ...
        const initApp = async () => {
            ...
            try {
                ...
                let savedId = mmkvStorage.getString(STORAGE_KEY_ID);
                if (!savedId) {
                    savedId = Math.random().toString(36).substr(2, 9).toUpperCase();
                    mmkvStorage.set(STORAGE_KEY_ID, savedId);
                }

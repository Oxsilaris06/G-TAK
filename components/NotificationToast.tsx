...
export const NotificationToast: React.FC<Props> = ({ message, type, isNightOps, onDismiss }) => {
  ...
  const getColors = () => {
    ...
    switch (type) {
      case 'alert':
        return { bg: '#450a0a', border: '#ef4444', text: '#fff', icon: '#ef4444' };
      case 'success':
        return { bg: '#052e16', border: '#22c55e', text: '#fff', icon: '#22c55e' };
      case 'warning':
        return { bg: '#422006', border: '#eab308', text: '#fff', icon: '#eab308' };
      default:
        return { bg: '#18181b', border: '#3b82f6', text: '#fff', icon: '#3b82f6' };
    }
  };
  ...
  return (
    ...
    >
      <MaterialIcons
        name={type === 'alert' ? 'warning' : type === 'success' ? 'check-circle' : type === 'warning' ? 'wifi-off' : 'info'}
        size={28}
        color={colors.icon}
      />
